...
/**
 * OBU Forms - Upload course custom fields (processor)
 *
...
class local_obu_forms_upload_processor {
    ...
    public function execute($tracker = null) {
        ...
        if (empty($tracker)) {
            $tracker = new local_obu_forms_upload_tracker(local_obu_forms_upload_tracker::NO_OUTPUT);
        }
        $tracker->start();
        ...
        while ($line = $this->cir->next()) {
            ...
            $course = new local_obu_forms_upload_course($data);
            if ($course->prepare()) {
                $course->proceed();
     ...
     *
     * This only returns parsed data, along with the errors.
     *
    ...
    public function preview($rows = 10, $tracker = null) {
        ...
        if (empty($tracker)) {
            $tracker = new local_obu_forms_tracker(local_obu_forms_tracker::NO_OUTPUT);
        }
        $tracker->start();
